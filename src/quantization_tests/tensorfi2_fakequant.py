...
class inject():
	def __init__(
		self, model, confFile, numFaults, log_level="ERROR", parallel_num=0, parallel_total=0,**kwargs, 
		):
		...
		#self.file = open("sanity_check/Session"+str(parallel_num)+"of"+str(parallel_total)+".txt", "a")
	...
	def layer_outputs(self, model, fiConf, **kwargs):
		...
		if(fiConf["Mode"] == "single"):
			...
			#Testing: change randnum to target specific layers
			randnum = 3 
			...
			global is_skipped
			if not is_skipped:
				for i in range((self.parallel_num-1)*self.numFaults*43*30):
					random.sample(range(num), fiSz)
					for i in range(fiSz):
						random.randint(0, 31)
				is_skipped=True
			...
			# Choose the indices for FI
			ind = random.sample(range(num), fiSz)
			#self.file.write(str(ind)+"\n")
			#print("Random Indice", ind)
			...
			if(fiFault == "zeros"):
				...
				for item in ind:
					...
					if(fiConf["Bit"] == "N"):
						pos = random.randint(0, 31)
						#self.file.write(str(pos)+"\n")
						#print("Random Bit Placement:",pos)
					else:
						pos = int(fiConf["Bit"])
					val_ = bitflip(val, pos)
			...
			#seed_skip(self.parallel_total-1, num, fiSz)
			return labels[0]
